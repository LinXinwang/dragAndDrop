<template>
  <div id="content" class="contentBox drop_area_one" @drop="outerDrop($event);" @dragover="allowDrop($event);">
    <div id="area1" class="drag_items_one" :style="{height: height_1+ 'px'}" draggable="true" @dragstart="dragStart($event);">
    ...
    <div id="area2" class="drag_items_one" :style="{height: height_2+ 'px'}" draggable="true" @dragstart="dragStart($event);" @dragend="outerDragEnd($event)">
      <div class="border_top"></div>
...
export default {
  ...
  data () {
    return {
      ...
      height_1: 500, // 容器一的高度
      height_2: 300, // 容器二的高度 （容器一二值允许改变高度）
      width_1: 400, // 子容器一的宽度
      width_2: 400, // 子容器二的宽度
      width_3: 400, // 子容器三的宽度（子容器一二三只允许改变宽度；高度跟着父容器高度变化） 
    }
  },
  mounted () {
    this.pageInit();
  ...
  methods: {
    ...
    registerMoveEvents () {
      this.addMouseEventForLeft();
      this.addMouseEventForRight();
      this.addMouseEventForTop();
      this.addMouseEventForBottom();
    ...
    dragStart (ev) {
      console.log("lxw dragStart----", ev.target.id);
      ev.stopPropagation();
      ...
      for (let i = 0; i < leftDoms.length; i++) {
        leftDoms[i].onmousedown = function () {
          ...
          document.onmousemove = function () {
            ...
            console.log("左边线上移动鼠标键", parentid, l);
            if (parentid == "p1") {
              // 不响应容器一的左边线的移动事件
            ...
            let localHeightAndWidth = {
              ...
              width_3: that.width_3,
            }
            window.localStorage.setItem("wh", JSON.stringify(localHeightAndWidth));
          }
        }
      }
    },
